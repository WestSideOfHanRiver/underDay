...
export default function TicketDetails({ setToggleDetail }: Props) {
    ...
    return (
        ...
        <form className={styles.form} onSubmit={handleSubmit(onSubmit)}>
            <p className={styles.required}>는 필수 입력 항목입니다.<br/>해당 내용을 입력해 개별 수강권을 만들어 주세요.</p>
        
            <div>
                <label className={styles.required}>사용자 ID</label>
                <input type="text" placeholder="사용자 ID를 입력해 주세요." {...register("user_numb", {required: true, maxLength: 16})} />
            </div>
            ...
            <div>
                ...
                <input type="text" placeholder="회원권 등록 회차를 입력해 주세요." {...register("umem_tnum", {required: true, maxLength: 6})} />
            </div>

            <div>
                <label className={styles.required}>사용 회차</label>
                <input type="text" placeholder="회원권 사용 회차를 입력해 주세요." {...register("umem_unum", {required: true, maxLength: 6})} />
            </div>

            <div>
                <label className={styles.required}>사용 여부</label>
                <div className={styles.radioWrap}>
                    <input type="radio" id="umem_yes" value="Y" {...register("umem_ysno", { required: true })} /> 
                    <label htmlFor="umem_yes">사용</label>

                    <input type="radio" id="umem_no" value="P" {...register("umem_ysno", { required: true })} />
                    <label htmlFor="umem_no">정지</label>

                    <input type="radio" id="umem_pause" value ="N" {...register("umem_ysno", { required: true })} />
            ...
            <div className={styles.btn_wrap}>
                <button type="button" className={styles.del_btn} onClick={() => setToggleDetail(false)}>닫기</button>
                <button type="submit" className={styles.submit_btn} onClick={() => setToggleDetail(true)}>회원권 생성하기</button>
            </div>
        </form>
        </>
    )
